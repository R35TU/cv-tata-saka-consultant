                      ...
                      try {
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Akun "${updated.name}" berhasil diperbarui.',
                              ),
                            ),
                          );
                        }
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Reset password untuk akun @${user.username}',
                style: const TextStyle(
                  fontFamily: 'Inter',
                  fontSize: 12,
                  color: Color(0xFF8E8E93),
              ...
              onPressed: isLoading
                  ...
                  : () async {
                      ...
                      try {
                        await ref
                            .read(authRepositoryProvider)
                            .resetUserPassword(user.id, newPwdCtrl.text.trim());
                        if (ctx.mounted) Navigator.of(ctx).pop();
                        if (mounted)
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Password @${user.username} berhasil direset.',
                              ),
                            ),
                          );
                      } catch (e) {
        ...
        content: SizedBox(
          ...
          child: relatedProjects.isEmpty
              ? const Padding(
                  ...
                  child: Text(
                    'Belum terdaftar di proyek manapun.',
                    style: TextStyle(
                      fontFamily: 'Inter',
                      fontSize: 13,
                      color: Color(0xFF8E8E93),
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (action) {
              switch (action) {
                ...
                case 'projects':
                  _showProjectRelationsDialog(user);
              }
            },
            itemBuilder: (ctx) => [
              const PopupMenuItem(
